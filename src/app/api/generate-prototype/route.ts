...
# テキストの品質ルール
- 日本語テキストの行変えが不自然にならないようにすること
- 例：「ご連絡くださ / い。」のように助詞・句点だけが次の行に落ちるのはNG
- 短いフレーズや見出しにはwhitespace-nowrapを適用して途中で折り返さないようにする
- ヒーローのキャッチコピーはtext-center + 適切なmax-widthで美しく折り返すこと

# カードレイアウトのルール（重要）
- 商品カード・料金カード・サービスカードなど、横並びのカードがある場合は必ずカードの高さを揃えること
- 各カードの価格やCTAボタンはカード下部に固定配置し、テキスト量が異なっても位置がずれないようにすること
- Tailwindのgridまたはflexboxで、各カードにh-fullとflex flex-colを適用し、最後の要素にmt-autoを使うパターンを推奨

# 人物写真と名前の整合性（重要）
- 日本の業種サイトなので、人物名は日本人名を使うこと
- 人物写真を使う場合は、名前の性別・国籍と写真の人物が一致するようにすること
- 例: 「田中和也」（日本人男性名）なら日本人男性の写真を使う。西洋人女性の写真は不可
- 人物写真にはUnsplashの風景や物の写真を使わず、適切なポートレート写真を選ぶこと
- 迷ったら人物写真は使わず、業種に関連する風景・物の写真にすること
